...
test('soleil et lune jaunes sur les thèmes, le chien au volant devant Maps Pro', async ({ page }) => {
  ...
  await expect(page.locator('.reglages-version img.reglages-pro-chien')).toHaveAttribute('src', '/icones/volant-96.png');
  // EN GRAND (GRAPH-2) : Armelin ne le distinguait pas à 22 px.
  const chien = (await page.locator('.reglages-version img.reglages-pro-chien').boundingBox())!;
